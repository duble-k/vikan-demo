...
const Lookup = () => {
  ...
  const handlePdfClick = async (pdfName) => {
    ...
    try {
      const response = await fetchPdf(searchInput, pdfName);
      if (response.ok) {
                ...
                <CardContent>
                  ...
                  <List>
                    ...
                    <ListItem>
                      <ListItemText
                        primary="Population"
                        secondary={countryInfo?.population}
                      />
                    </ListItem>
                    <Divider />
                    <ListItem>
                      <ListItemText
                        primary="Political"
                        secondary={countryInfo?.political}
                      />
                    </ListItem>
                    <Divider />
                    <ListItem>
                      <ListItemText
                        primary="Currency"
                        secondary={countryInfo?.currency}
                      />
                    </ListItem>
                  </List>
                </CardContent>
          ...
          {countryInfo.associatedPdfs.length > 0 && (
            <Card raised>
              <CardContent>
                  ...
                  {countryInfo?.associatedPdfs
                    ...
                    .map((pdfName, index) => (
                      <div key={index}>
                        ...
                        {index < countryInfo.associatedPdfs.length - 1 && (
                          <Divider />
                        )}
                      </div>
                    ))}
                ...
                <Pagination
                  count={Math.ceil(
                    countryInfo.associatedPdfs.length / itemsPerPage
                  )}
                  page={page}
